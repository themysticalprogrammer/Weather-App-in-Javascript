...
function fetchData(locationValue, index) {
	fetch(
		`http://api.weatherapi.com/v1/current.json?key=af2f5df9717e49f795895037210411&q=${locationValue}&aqi=no`
	)
		.then((response) => {
			if (response.status === 200) {
				...
				<div class="card my-3" style="width: 80vw;">
 				...
 				`;
					...
					.catch(() => {
						document.getElementsByClassName('container')[0].innerHTML =
							'<h2 class="text-center">Some Error Occured</h2>';
					});
			} else if (response.status === 400) {
				alert('Sorry Add a Valid Location');
				// let locationArray = JSON.parse(localStorage.getItem('added-location'));
				// console.log(locationArray);
				// locationArray.pop();
				// console.log(locationArray);
				// localStorage.setItem(JSON.stringify(locationArray));
				// showWeatherCards();
			} else {
				document.getElementsByClassName('container')[0].innerHTML =
					'<h2 class="text-center">Some Error Occured</h2>';
